...
The **R** package called`tinyteX` is still a relatively new project which strives to offer all the functionality of $\LaTeX$, but remain lightweight and easier to maintain compared to full installations of `MacTeX` or `MikTeX`.  I am now using this in place of LaTeX!

```{block type='rmdlink', echo=TRUE}
Find out more about `tinytex` at https://yihui.org/tinytex/
```

To download and install `tinytex` you will need to copy-paste the following code into your console *(or a code chunk)* and run it. This only needs to be done once per computer.
...
```{block type='rmddownload', echo=TRUE, include=FALSE}
