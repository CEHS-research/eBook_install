...
```{block type='rmdlink', echo=TRUE}
$R Studio$ has an entire [online learning library](https://education.rstudio.com/).  It is full of helpful links and resources for R programing, R markdown, and statistics.
```
...
```{block type='rmdlink', echo=TRUE}
Keep an eye on the developing `tinytex` package by checking back to it's [home page](https://yihui.name/tinytex/).
```